...
class Utility {
   ...
   * @param {Number} b
   */
  static approximatelyEqual(a2, b2, epsilon = 1e-8) {
   ...
   * @returns {Coordinates}
   */
  static snapToGrid(x2, y3, gridSize) {
    if (gridSize === 1) {
      return [x2, y3];
    }
    return [
      gridSize * Math.floor(x2 / gridSize),
      gridSize * Math.floor(y3 / gridSize)
    ];
  }
  /**
   * @template T
   ...
   * @returns {T[]}
   */
  static mergeArrays(reference = [], additional = [], predicate = (l2, r2) => l2 == r2, adding = (v2) => {
  }) {
    let result = [];
    reference = [...reference];
    ...
    restart:
      while (true) {
        for (let j = 0; j < additional.length; ++j) {
          for (let i4 = 0; i4 < reference.length; ++i4) {
            if (predicate(reference[i4], additional[j])) {
              result.push(
                ...reference.splice(0, i4),
                ...additional.splice(0, j).map((v2) => (adding(v2), v2)),
    ...
    result.push(
      ...additional.filter((vb) => !result.some((vr) => predicate(vr, vb))).map((v2, k2) => (adding(v2), v2))
    );
    return result;
  }
  /** @param {String} value */
  ...
  static formatStringName(value = "") {
    return value.replace(/^\s*b(?=[A-Z])/, "").replaceAll(Configuration.nameRegexSpaceReplacement, " ").trim().split(" ").map((v2) => Utility.capitalFirstLetter(v2)).join(" ");
  }
  /** @param {String} value */
   ...
   * @returns {Coordinates}
   */
  static getPolarCoordinates(x2, y3, positiveTheta = false) {
    let theta = Math.atan2(y3, x2);
    if (positiveTheta && theta < 0) {
      theta = 2 * Math.PI + theta;
    }
    return [
      Math.sqrt(x2 * x2 + y3 * y3),
      theta
    ];
  }
  /**
   ...
   * @returns {Coordinates}
   */
  static getCartesianCoordinates(r2, theta) {
    return [
      r2 * Math.cos(theta),
      r2 * Math.sin(theta)
    ];
  ...
  static range(begin = 0, end = 0, step = end >= begin ? 1 : -1) {
    return Array.from({ length: Math.ceil((end - begin) / step) }, (_2, i4) => begin + i4 * step);
  }
  /** @param {String[]} words */
...
const t$2 = globalThis, e$3 = t$2.ShadowRoot && (void 0 === t$2.ShadyCSS || t$2.ShadyCSS.nativeShadow) && "adoptedStyleSheets" in Document.prototype && "replace" in CSSStyleSheet.prototype, s$2 = /* @__PURE__ */ Symbol(), o$4 = /* @__PURE__ */ new WeakMap();
let n$3 = class n {
...
const { is: i$4, defineProperty: e$2, getOwnPropertyDescriptor: h$1, getOwnPropertyNames: r$1, getOwnPropertySymbols: o$3, getPrototypeOf: n$2 } = Object, a$1 = globalThis, c$1 = a$1.trustedTypes, l$1 = c$1 ? c$1.emptyScript : "", p$9 = a$1.reactiveElementPolyfillSupport, d$1 = (t2, s2) => t2, u$1 = { toAttribute(t2, s2) {
  switch (s2) {
    case Boolean:
      t2 = t2 ? l$1 : null;
      break;
...
y$1.elementStyles = [], y$1.shadowRootOptions = { mode: "open" }, y$1[d$1("elementProperties")] = /* @__PURE__ */ new Map(), y$1[d$1("finalized")] = /* @__PURE__ */ new Map(), p$9?.({ ReactiveElement: y$1 }), (a$1.reactiveElementVersions ??= []).push("2.1.2");
const t$1 = globalThis, i$3 = (t2) => t2, s$1 = t$1.trustedTypes, e$1 = s$1 ? s$1.createPolicy("lit-html", { createHTML: (t2) => t2 }) : void 0, h = "$lit$", o$2 = `lit$${Math.random().toFixed(9).slice(2)}$`, n$1 = "?" + o$2, r = `<${n$1}>`, l = document, c = () => l.createComment(""), a = (t2) => null === t2 || "object" != typeof t2 && "function" != typeof t2, u = Array.isArray, d = (t2) => u(t2) || "function" == typeof t2?.[Symbol.iterator], f = "[ 	\n\f\r]", v = /<(?:(!--|\/[^a-zA-Z])|(\/?[a-zA-Z][^>\s]*)|(\/?$))/g, _ = /-->/g, m = />/g, p$8 = RegExp(`>|${f}(?:([^\\s"'>=/]+)(${f}*=${f}*(?:[^ 	
...
const N = (t2, i4) => {
  ...
  for (let i5 = 0; i5 < s2; i5++) {
    const s3 = t2[i5];
    let a2, u2, d2 = -1, f2 = 0;
    for (; f2 < s3.length && (c2.lastIndex = f2, u2 = c2.exec(s3), null !== u2); ) f2 = c2.lastIndex, c2 === v ? "!--" === u2[1] ? c2 = _ : void 0 !== u2[1] ? c2 = m : void 0 !== u2[2] ? (y2.test(u2[2]) && (n3 = RegExp("</" + u2[2], "g")), c2 = p$8) : void 0 !== u2[3] && (c2 = p$8) : c2 === p$8 ? ">" === u2[0] ? (c2 = n3 ?? v, d2 = -1) : void 0 === u2[1] ? d2 = -2 : (d2 = c2.lastIndex - u2[2].length, a2 = u2[1], c2 = void 0 === u2[3] ? p$8 : '"' === u2[3] ? $ : g) : c2 === $ || c2 === g ? c2 = p$8 : c2 === _ || c2 === m ? c2 = v : (c2 = p$8, n3 = void 0);
    const x2 = c2 === p$8 && t2[i5 + 1].startsWith("/>") ? " " : "";
    l2 += c2 === v ? s3 + r : d2 >= 0 ? (e2.push(a2), s3.slice(0, d2) + h + s3.slice(d2) + o$2 + x2) : s3 + o$2 + (-2 === d2 ? i5 : x2);
  }
...
class IElement extends i$2 {
  ...
  connectedCallback() {
    ...
    this.blueprint = /** @type {Blueprint} */
    this.closest("ueb-blueprint");
...
class Parser {
  ...
  static frame(value, label = "", indentation = "") {
    ...
    const width = Math.max(...rows.map((r2) => r2.length));
    const rightPadding = width < label.length ? " ".repeat(label.length - width) : "";
    for (let i4 = 0; i4 < rows.length; ++i4) {
      rows[i4] = indentation + "| " + rows[i4] + " ".repeat(width - rows[i4].length) + rightPadding + " |";
    }
  ...
  isHighlighted(context, path) {
    ...
    let a2, b2;
    for (a2 = path, b2 = /** @type {PathNode} */
    context.highlighted; a2.current && b2.current; a2 = a2.parent, b2 = b2.parent) {
      if (a2.current !== b2.current || a2.index !== b2.index) {
        return false;
      }
    }
    return !a2.current && !b2.current;
  }
  /** @param {PathNode?} path */
  isVisited(path) {
    if (!path) {
...
class StringParser extends Parser {
  ...
  parse(context, position, path, index) {
    ...
    const result = this.#value === value ? Reply.makeSuccess(end, this.#value, path, end) : Reply.makeFailure();
    return result;
  }
  /**
   * @protected
...
class AlternativeParser extends Parser {
  ...
  parse(context, position, path, index) {
    ...
    for (let i4 = 0; i4 < this.#parsers.length; ++i4) {
      const outcome = this.#parsers[i4].parse(context, position, path, i4);
  ...
  doToString(context, indentation, path, index) {
    if (this.#parsers.length === 2 && this.#parsers[1] instanceof SuccessParser) {
      let result2 = this.#parsers[0].toString(context, indentation, path, 0);
    ...
    let result = "ALT<\n" + deeperIndentation + this.#parsers.map((parser, i4) => parser.toString(
      context,
      deeperIndentation + " ".repeat(i4 === 0 ? 0 : Parser.indentation.length - 2),
      path,
      i4
    )).join("\n" + deeperIndentation + "| ") + "\n" + indentation + ">";
    return result;
...
class ChainedParser extends Parser {
  ...
  parse(context, position, path, index) {
    ...
    const result = this.#fn(outcome.value, context.input, outcome.position).getParser().parse(context, outcome.position, path, 0);
    if (outcome.bestPosition > result.bestPosition) {
      result.bestParser = outcome.bestParser;
      result.bestPosition = outcome.bestPosition;
    }
...
class Lookahead extends Parser {
  ...
  parse(context, position, path, index) {
    ...
    result = result.status == (this.#type === Lookahead.Type.POSITIVE_AHEAD) ? Reply.makeSuccess(position, "", path, position) : Reply.makeFailure();
    return result;
  }
  /**
   * @protected
...
class RegExpParser extends Parser {
  ...
  parse(context, position, path, index) {
    ...
    const result = match ? Reply.makeSuccess(position, this.#matchMapper(match), path, position) : Reply.makeFailure();
    return result;
  }
  /**
   * @protected
  ...
  doToString(context, indentation, path, index) {
    ...
    const shortname = Object.entries(RegExpParser.common).find(([k2, v2]) => v2.source === this.#regexp.source)?.[0];
    if (shortname) {
      result = "P." + shortname;
    }
    return result;
...
class MapParser extends Parser {
  ...
  parse(context, position, path, index) {
    ...
    const result = (
      /** @type {Result<T>} */
      this.#parser.parse(context, position, path, 0)
    );
    if (result.status) {
  ...
  doToString(context, indentation, path, index) {
    ...
    if (this.#parser instanceof RegExpParser) {
      if (Object.values(RegExpParser.common).includes(this.#parser.regexp)) {
        if (this.#parser.regexp === RegExpParser.common.numberInteger && this.#mapper === /** @type {(v: any) => BigInt} */
        BigInt) {
...
class SequenceParser extends Parser {
  ...
  parse(context, position, path, index) {
    ...
    const value = (
      /** @type {ParserValue<T>} */
      new Array(this.#parsers.length)
    );
    const result = Reply.makeSuccess(position, value);
    for (let i4 = 0; i4 < this.#parsers.length; ++i4) {
      const outcome = this.#parsers[i4].parse(context, result.position, path, i4);
      if (outcome.bestPosition > result.bestPosition) {
        result.bestParser = outcome.bestParser;
        result.bestPosition = outcome.bestPosition;
      }
  ...
  doToString(context, indentation, path, index) {
    const deeperIndentation = indentation + Parser.indentation;
    const result = "SEQ<\n" + deeperIndentation + this.#parsers.map((parser, index2) => parser.toString(context, deeperIndentation, path, index2)).join("\n" + deeperIndentation) + "\n" + indentation + ">";
    return result;
  }
}
class TimesParser extends Parser {
  ...
  doToString(context, indentation, path, index) {
    let result = this.parser.toString(context, indentation, path, 0);
    const serialized = this.#min === 0 && this.#max === 1 ? "?" : this.#min === 0 && this.#max === Number.POSITIVE_INFINITY ? "*" : this.#min === 1 && this.#max === Number.POSITIVE_INFINITY ? "+" : "{" + this.#min + (this.#min !== this.#max ? "," + this.#max : "") + "}";
    result += serialized;
    return result;
  }
}
class Parsernostrum {
  ...
  static #firstElementGetter = ([v2, _2]) => v2;
  /** @param {[any, any, ...any] | RegExpExecArray} param0 */
  static #secondElementGetter = ([_2, v2]) => v2;
   ...
   */
  // @ts-expect-error
  static #joiner = (v2) => v2 instanceof Array ? v2.join("") : v2;
  // Prefedined parsers
  ...
  static #simplifyPath(path) {
    ...
    for (let i4 = 1; i4 < array.length; ++i4) {
      const existing = visited.get(array[i4].current);
      if (existing !== void 0) {
        if (array[i4 + 1]) {
          array[i4 + 1].parent = array[existing];
        }
        visited = new Map([...visited.entries()].filter(([parser, index]) => index <= existing || index > i4));
        visited.set(array[i4].current, existing);
        array.splice(existing + 1, i4 - existing);
        i4 = existing;
      } else {
        visited.set(array[i4].current, i4);
      }
    }
  ...
  parse(input, printParser = true) {
    ...
    const inlineInput = input.replaceAll(
      /^(\s)+|\s{6,}|\s*?\n\s*/g,
      (m2, startingSpace, offset2) => {
        let replaced = startingSpace ? "..." : " ... ";
        if (offset2 <= result.bestPosition) {
          if (result.bestPosition < offset2 + m2.length) {
            bestPosition -= result.bestPosition - offset2;
          } else {
            bestPosition -= m2.length - replaced.length;
          }
        }
        return replaced;
      }
...
` + (printParser ? "\n" + (result.bestParser ? "Last valid parser matched:" : "No parser matched:") + bestParser + "\n" : "")
   ...
   */
  static seq(...parsers) {
    return new this(new SequenceParser(...parsers.map((p2) => p2.getParser())));
  }
  /**
   * @template {Parsernostrum[]} P
   * @param {P} parsers
   ...
   */
  static alt(...parsers) {
    return new this(new AlternativeParser(...parsers.map((p2) => p2.getParser())));
  }
  /**
   * @template {Parsernostrum} P
   * @param {P} parser
  ...
  /** @param {Number} n */
  atLeast(n3) {
    return this.times(n3, Number.POSITIVE_INFINITY);
  }
  /** @param {Number} n */
  atMost(n3) {
  ...
  assert(fn) {
    return this.chain(
      (v2, input, position) => fn(v2, input, position) ? Parsernostrum.success().map(() => v2) : Parsernostrum.failure()
    );
  }
  join(value = "") {
...
class IEntity {
  ...
  constructor(values = {}) {
    ...
    const keys = Utility.mergeArrays(
      Object.keys(values),
      Object.entries(attributes).filter(([k2, v2]) => v2.default !== void 0).map(([k2, v2]) => k2)
    );
  ...
  doSerialize(insideString = false, indentation = "", Self = (
    ...
    for (const key of keys) {
      ...
      if (value.inlined) {
        const inlinedPrintKey = valueType.className() === "ArrayEntity" ? (k2) => printKey(`${keyValue}${k2}`) : (k2) => printKey(`${keyValue}.${k2}`);
  ...
  equals(other) {
    ...
    for (let i4 = 0; i4 < thisKeys.length; ++i4) {
      const k2 = thisKeys[i4];
      if (!otherKeys.includes(k2)) {
        return false;
      }
      const a2 = this[k2];
      const b2 = other[k2];
      if (a2 instanceof IEntity) {
        if (!a2.equals(b2)) {
          return false;
        }
      } else if (a2 instanceof Array && b2 instanceof Array) {
        if (a2.length !== b2.length) {
          return false;
        }
        for (let j = 0; j < a2.length; ++j) {
          if (!(a2[j] instanceof IEntity && a2[j].equals(b2[j])) && a2[j] !== b2[j]) {
            return false;
          }
        }
      } else {
...
class BooleanEntity extends IEntity {
  ...
  static createGrammar() {
    return Parsernostrum.regArray(/(true)|(True)|(false)|(False)/).map((v2) => {
      const result = v2[1] ?? v2[2] ? new this(true) : new this(false);
      result.uppercase = (v2[2] ?? v2[4]) !== void 0;
      return result;
    }).label("BooleanEntity");
  }
...
class ElementFactory {
  ...
  static #elementConstructors = /* @__PURE__ */ new Map();
  /**
...
class AlternativesEntity extends IEntity {
  ...
  static className() {
    ...
    if (this.alternatives.length) {
      result += ".accepting(" + this.alternatives.map((v2) => v2.className()).join(", ") + ")";
    }
    return result;
  }
  static createGrammar() {
...
class Grammar {
  ...
  ).map(([_0, n3, plusInf, minusInf]) => n3 ? Number(n3) : plusInf ? Number.POSITIVE_INFINITY : Number.NEGATIVE_INFINITY);
  // @ts-expect-error
  static bigInt = Parsernostrum.reg(new RegExp(Parsernostrum.number.getParser().parser.regexp.source)).map(BigInt).map(
    ...
    return Parsernostrum.seq(
      ...
      return grammar.map(
        (attributeValue2) => (values) => {
          ...
          attributeKey.reduce(
            (acc, cur, i4) => {
              acc[cur]["inlined"] = inlined && i4 < attributeKey.length - 1;
              return acc[cur];
            },
            values
          );
  ...
  static createEntityGrammar(entityType, entriesSeparator = this.commaSeparation, completeness = null, minKeys = 1) {
    ...
    return Parsernostrum.seq(
      ...
      return completeness != null ? Parsernostrum.success().assert(
        (v2) => keys.filter((k2) => expectedKeys.includes(k2)).length / expectedKeys.length >= completeness
      ).map(() => new entityType(values)) : Parsernostrum.success().map(() => new entityType(values));
    });
  }
}
class ArrayEntity extends IEntity {
  ...
  static createGrammar(elementGrammar = this.type?.grammar ?? Parsernostrum.lazy(() => this.unknownEntityGrammar)) {
    return this.inlined ? elementGrammar : Parsernostrum.seq(
      Parsernostrum.reg(/\(\s*/),
  ...
  doSerialize(insideString = false, indentation = "", Self = (
    ...
    let result = this.values.map((v2) => v2?.serialize(insideString)).join(Self.attributeSeparator);
    if (this.trailing) {
  ...
  equals(other) {
    ...
    for (let i4 = 0; i4 < this.values.length; ++i4) {
      if (!this.values[i4].equals(other.values[i4])) {
        return false;
      }
    }
    return true;
...
class GuidEntity extends IEntity {
  ...
  static generateGuid() {
    ...
    let guid = "";
    values.forEach((n3) => {
      guid += ("0".repeat(8) + n3.toString(16).toUpperCase()).slice(-8);
    });
    return guid;
  }
  ...
  static createGrammar() {
    return Parsernostrum.reg(/[0-9A-F]{32}/i).map((v2) => new this(v2)).label("GuidEntity");
  }
  serialize(insideString = false, indentation = "", Self = (
    /** @type {typeof IEntity} */
    this.constructor
...
class ObjectReferenceEntity extends IEntity {
  ...
  constructor(type = "None", path = "", serializer = type.includes("/") || path ? (t2, p2) => `"${t2 + (p2 ? `'${p2}'` : "")}"` : (t2, p2) => t2) {
    super();
    this.#type = type;
    this.#path = path;
  ...
  static createFullReferenceSerializedGrammar() {
    ...
    ).map(([_0, type, path]) => new this(type, path, (t2, p2) => `"${t2}${p2 ? `'${p2}'` : ""}"`));
  }
  /** @returns {P<ObjectReferenceEntity>} */
  static createTypeReferenceGrammar() {
    return this.typeReference.map((v2) => new this(v2, "", (t2, p2) => t2));
  }
...
class StringEntity extends IEntity {
  ...
  static createGrammar() {
    return Parsernostrum.doubleQuotedString.map((insideString) => new this(StringEntity.unescape(insideString))).label("StringEntity");
  }
  /** @param {String} value */
...
class NumberEntity extends IEntity {
  ...
  static createGrammar() {
    return Parsernostrum.regArray(
      ...
      ({ 2: precision, groups: { n: n3, posInf, negInf } }) => new this(
        n3 ? Number(n3) : posInf ? Number.POSITIVE_INFINITY : Number.NEGATIVE_INFINITY,
        precision?.length
      )
    ).label("NumberEntity");
  }
...
class IntegerEntity extends NumberEntity {
  ...
  static createGrammar() {
    return Parsernostrum.numberInteger.map((v2) => new this(v2));
  }
}
class ColorChannelEntity extends IEntity {
  ...
  static createGrammar() {
    return Parsernostrum.number.map((v2) => new this(v2));
  }
  serialize(insideString = false, indentation = "", Self = (
    /** @type {typeof IEntity} */
    this.constructor
...
class LinearColorEntity extends IEntity {
  ...
  static linearToSRGB(x2) {
    if (x2 <= 0) {
      ...
      return Math.pow(x2, 1 / 2.4) * 1.055 - 0.055;
    }
  }
  /** @param {Number} x */
  static sRGBtoLinear(x2) {
    if (x2 <= 0) {
      return 0;
    } else if (x2 >= 1) {
      return 1;
      ...
      return Math.pow((x2 + 0.055) / 1.055, 2.4);
    }
  }
  static getWhite() {
  ...
  static getLinearColorFromHexGrammar() {
    ...
    )).map(([m2, R2, G, B2, A2]) => new this({
      R: parseInt(R2, 16) / 255,
      G: parseInt(G, 16) / 255,
      B: parseInt(B2, 16) / 255,
      A: parseInt(A2 ?? "FF", 16) / 255
    }));
  }
  ...
  #updateHSV() {
    const r2 = this.R.value;
    const g2 = this.G.value;
    const b2 = this.B.value;
    if (Utility.approximatelyEqual(r2, g2) && Utility.approximatelyEqual(r2, b2) && Utility.approximatelyEqual(g2, b2)) {
      this.S.value = 0;
      this.V.value = r2;
      return;
    }
   ...
   * @param {Number} a
   */
  setFromHSVA(h2, s2, v2, a2 = 1) {
    const i4 = Math.floor(h2 * 6);
    const f2 = h2 * 6 - i4;
    const p2 = v2 * (1 - s2);
    const q = v2 * (1 - f2 * s2);
    const t2 = v2 * (1 - (1 - f2) * s2);
    const values = [v2, q, p2, p2, t2, v2];
    const [r2, g2, b2] = [values[i4 % 6], values[(i4 + 4) % 6], values[(i4 + 2) % 6]];
    this.R.value = r2;
    this.G.value = g2;
    this.B.value = b2;
    this.A.value = a2;
   ...
   * @param {Number} a
   */
  setFromWheelLocation(x2, y3, v2, a2) {
    const [r2, theta] = Utility.getPolarCoordinates(x2, y3, true);
    this.setFromHSVA(1 - theta / (2 * Math.PI), r2, v2, a2);
  }
  ...
  toCSSRGBValues() {
    const r2 = Math.round(this.R.value * 255);
    const g2 = Math.round(this.G.value * 255);
    const b2 = Math.round(this.B.value * 255);
    return cssValue`${r2}, ${g2}, ${b2}`;
  }
  toRGBA() {
    return [
  ...
  toRGBAString() {
    return this.toRGBA().map((v2) => v2.toString(16).toUpperCase().padStart(2, "0")).join("");
  }
  toSRGBAString() {
    return this.toSRGBA().map((v2) => v2.toString(16).toUpperCase().padStart(2, "0")).join("");
  }
  toHSVA() {
...
class MirroredEntity extends IEntity {
  ...
  static createGrammar(elementGrammar = this.type?.grammar ?? Parsernostrum.lazy(() => this.unknownEntityGrammar)) {
    return this.type?.grammar.map((v2) => new this(() => v2));
  }
  /**
   * @template {typeof IEntity} T
   * @this {T}
...
class NaturalNumberEntity extends IntegerEntity {
  ...
  static createGrammar() {
    return Parsernostrum.numberNatural.map((v2) => new this(v2));
  }
}
class NullEntity extends IEntity {
  ...
  static createGrammar() {
    return Parsernostrum.reg(new RegExp(String.raw`\(${Parsernostrum.whitespaceInlineOpt.getParser().regexp.source}\)`)).map((v2) => new this()).label("NullEntity");
  }
  serialize(insideString = false, indentation = "", Self = (
    /** @type {typeof IEntity} */
    this.constructor
...
class ByteEntity extends IntegerEntity {
  ...
  createGrammar() {
    return Parsernostrum.numberByte.map((v2) => new this(v2));
  }
}
...
class SymbolEntity extends IEntity {
  ...
  static createGrammar() {
    return Grammar.symbol.map((v2) => new this(v2)).label("SymbolEntity");
  }
  constructor(value = "") {
    super();
    this.value = value;
...
class EnumEntity extends SymbolEntity {
  ...
  static createGrammar() {
    return Grammar.symbol.map((v2) => new this(v2));
  }
}
class EnumDisplayValueEntity extends EnumEntity {
  ...
  static createGrammar() {
    return Parsernostrum.reg(Grammar.Regex.InsideString).map((v2) => new this(v2));
  }
}
class InvariantTextEntity extends IEntity {
  ...
  static createGrammar() {
    return Parsernostrum.alt(
      ...
      Parsernostrum.reg(new RegExp(this.lookbehind)).map(() => "")
...
class FormatTextEntity extends IEntity {
  ...
  doSerialize(insideString = false, indentation = "", Self = (
    ...
    return this.lookbehind + "(" + this.values.map((v2) => v2.serialize(insideString)).join(separator) + (Self.trailing ? separator : "") + ")";
  }
  toString() {
    ...
    const values = this.values.slice(1).map((v2) => v2?.valueOf());
...
class Integer64Entity extends IEntity {
  ...
  static createGrammar() {
    return Parsernostrum.numberBigInteger.map((v2) => new this(v2));
  }
  serialize(insideString = false, indentation = "", Self = (
    /** @type {typeof IEntity} */
    this.constructor
...
class RBSerializationVector2DEntity extends Vector2DEntity {
  ...
  static createGrammar() {
    return Parsernostrum.alt(
      ...
      )).map(({ groups: { x: x2, y: y3 } }) => new this({
        X: new Vector2DEntity.attributes.X(x2),
        Y: new Vector2DEntity.attributes.Y(y3)
      })),
      Vector2DEntity.grammar.map((v2) => new this({
        X: v2.X,
        Y: v2.Y
      }))
...
class SimpleSerializationRotatorEntity extends RotatorEntity {
  ...
  static createGrammar() {
    return Parsernostrum.alt(
      ...
      )).map(([_2, p2, pPrecision, y3, yPrecision, r2, rPrecision]) => new this({
        R: new RotatorEntity.attributes.R(r2, rPrecision?.length),
        P: new RotatorEntity.attributes.P(p2, pPrecision?.length),
        Y: new RotatorEntity.attributes.Y(y3, yPrecision?.length)
      })),
      RotatorEntity.grammar.map((v2) => new this({
        R: v2.R,
        P: v2.P,
        Y: v2.Y
      }))
...
class SimpleSerializationVector2DEntity extends Vector2DEntity {
  ...
  static createGrammar() {
    return Parsernostrum.alt(
      ...
      )).map(([_2, x2, xPrecision, y3, yPrecision]) => new this({
        X: new Vector2DEntity.attributes.X(x2, xPrecision?.length),
        Y: new Vector2DEntity.attributes.Y(y3, yPrecision?.length)
      })),
      Vector2DEntity.grammar.map((v2) => new this({
        X: v2.X,
        Y: v2.Y
...
class SimpleSerializationVector4DEntity extends Vector4DEntity {
  ...
  static createGrammar() {
    ...
    return Parsernostrum.alt(
      ...
      )).map(([_0, x2, y3, z2, w]) => new this({
        X: new Vector4DEntity.attributes.X(x2),
        Y: new Vector4DEntity.attributes.Y(y3),
        Z: new Vector4DEntity.attributes.Z(z2),
...
class SimpleSerializationVectorEntity extends VectorEntity {
  ...
  static createGrammar() {
    return Parsernostrum.alt(
      ...
      )).map(([_2, x2, xPrecision, y3, yPrecision, z2, zPrecision]) => new this({
        X: new VectorEntity.attributes.X(x2, xPrecision?.length),
        Y: new VectorEntity.attributes.Y(y3, yPrecision?.length),
        Z: new VectorEntity.attributes.Z(z2, zPrecision?.length)
      })),
      VectorEntity.grammar.map((v2) => new this({
        X: v2.X,
        Y: v2.Y,
        Z: v2.Z
      }))
...
class ObjectEntity extends IEntity {
  ...
  /** @type {(k: String) => String} */
  static printKey = (k2) => !k2.startsWith(Configuration.subObjectAttributeNamePrefix) ? k2 : "";
  static attributeSeparator = "\n";
  ...
  static inlinedArrayEntryGrammar = Parsernostrum.seq(
    Parsernostrum.alt(
      Grammar.symbolQuoted.map((v2) => [v2, true]),
      Grammar.symbol.map((v2) => [v2, false])
    ),
  ...
  constructor(values = {}) {
    ...
    let inputIndex = 0;
    let outputIndex = 0;
    this.getCustomproperties().forEach((pinEntity, i4) => {
      pinEntity.objectEntity = this;
      pinEntity.pinIndex = pinEntity.isInput() ? inputIndex++ : pinEntity.isOutput() ? outputIndex++ : i4;
    });
  ...
  static createSubObjectGrammar() {
    return Parsernostrum.lazy(() => this.grammar).map(
      (object) => (values) => {
  ...
  /** @returns {PinEntity[]} */
  getPinEntities() {
    return this.getCustomproperties().filter((v2) => v2.constructor === PinEntity);
  }
  /** @returns {ObjectEntity[]} */
  getSubobjects() {
    return Object.keys(this).filter((k2) => k2.startsWith(Configuration.subObjectAttributeNamePrefix)).flatMap((k2) => [this[k2], .../** @type {ObjectEntity} */
    this[k2].getSubobjects()]);
  }
  switchTarget() {
...
class KnotEntity extends ObjectEntity {
   ...
   */
  constructor(values = {}, pinReferenceForType = void 0) {
...
class KeyboardShortcut extends IInput {
   ...
   * @param {Options} options
   */
  constructor(target, blueprint, options = {}, onKeyDown = KeyboardShortcut.#ignoreEvent, onKeyUp = KeyboardShortcut.#ignoreEvent) {
    ...
    options.activationKeys = options.activationKeys.map((v2) => {
      if (v2 instanceof KeyBindingEntity) {
        return v2;
      }
      if (v2.constructor === String) {
        const parsed = KeyBindingEntity.grammar.run(v2);
        if (parsed.status) {
          return parsed.value;
        }
      }
    ...
    this.keyDownHandler = (e2) => {
      if (self.#activationKeys.some(
        (keyEntry) => wantsShift(keyEntry) == e2.shiftKey && wantsCtrl(keyEntry) == e2.ctrlKey && wantsAlt(keyEntry) == e2.altKey && Configuration.Keys[keyEntry.Key.value] == e2.code
      )) {
        if (this.consumeEvent) {
          e2.preventDefault();
          e2.stopImmediatePropagation();
        }
        this.pressedKey = e2.code;
        self.fire();
        document.removeEventListener("keydown", self.keyDownHandler);
        document.addEventListener("keyup", self.keyUpHandler);
      }
    };
    this.keyUpHandler = (e2) => {
      if (self.#activationKeys.some(
        (keyEntry) => keyEntry.bShift?.valueOf() && e2.key == "Shift" || keyEntry.bCtrl?.valueOf() && e2.key == "Control" || keyEntry.bAlt?.valueOf() && e2.key == "Alt" || keyEntry.bCmd?.valueOf() && e2.key == "Meta" || Configuration.Keys[keyEntry.Key.value] == e2.code
      )) {
        if (this.consumeEvent) {
          e2.stopImmediatePropagation();
        }
...
class IPointing extends IInput {
  ...
  setLocationFromEvent(mouseEvent) {
    ...
    location = this.options.ignoreTranslateCompensate ? location : this.blueprint.compensateTranslation(location[0], location[1]);
...
class MouseClick extends IPointing {
  ...
  #mouseDownHandler = (e2) => {
    ...
    switch (e2.button) {
      case this.options.clickButton:
        if (!this.options.strictTarget || e2.target === e2.currentTarget) {
          if (this.consumeEvent) {
            e2.stopImmediatePropagation();
          }
  ...
  /** @param {MouseEvent} e */
  #mouseUpHandler = (e2) => {
    if (!this.options.exitAnyButton || e2.button == this.options.clickButton) {
      if (this.consumeEvent) {
        e2.stopImmediatePropagation();
      }
   ...
   * @param {Options} options
   */
  constructor(target, blueprint, options = {}, onClick = MouseClick.#ignoreEvent, onUnclick = MouseClick.#ignoreEvent) {
  ...
  listenEvents() {
    ...
    if (this.options.clickButton === Configuration.mouseRightClickButton) {
      this.target.addEventListener("contextmenu", (e2) => e2.preventDefault());
    }
  }
  unlistenEvents() {
...
class MouseDbClick extends IPointing {
  ...
  /** @param {MouseEvent} e */
  #mouseDbClickHandler = (e2) => {
    if (!this.options.strictTarget || e2.target === e2.currentTarget) {
      if (this.consumeEvent) {
        e2.stopImmediatePropagation();
      }
      this.clickedPosition = this.setLocationFromEvent(e2);
...
class Shortcuts {
  ...
  static enableZoomIn = ["LeftControl", "RightControl"];
  // Button to enable more than 1:1 zoom
...
class ITemplate {
  ...
  setup() {
    this.#inputObjects.forEach((v2) => v2.setup());
  }
  cleanup() {
    this.#inputObjects.forEach((v2) => v2.cleanup());
  }
  /** @param {PropertyValues} changedProperties */
  willUpdate(changedProperties) {
  }
...
class LinkTemplate extends IFromToPositionedTemplate {
  /** @param {Number} x */
  static sigmoidPositive(x2, curvature = 3.7, length = 1.1) {
    return 1 - Math.exp(-((x2 / length) ** curvature));
  }
  /**
   ...
   */
  static decreasingValue(m2, p2) {
    const a2 = -m2 * p2[0] ** 2;
    const q = p2[1] - a2 / p2[0];
    return (x2) => a2 / x2 + q;
  }
  static clampedLine = (x2) => Math.min(Math.max(0, x2), 1);
  ...
  static c2Clamped = (x2) => -40 * LinkTemplate.clampedLine(x2 / 200) + 80;
  #uniqueId = `ueb-id-${Math.floor(Math.random() * 1e12)}`;
  /** @param {Coordinates} location */
  #createKnot = (location) => {
    const knotEntity = new KnotEntity({}, this.element.origin.entity);
    const knot = (
      /** @type {NodeElementConstructor} */
      ElementFactory.getConstructor("ueb-node").newObject(knotEntity)
    );
    ...
    this.blueprint.addGraphElement(knot);
    ...
    const link = (
      /** @type {LinkElementConstructor} */
      ElementFactory.getConstructor("ueb-link").newObject(outputPin, knotTemplate.inputPin)
    );
  ...
  #calculateSVGPath(changedProperties) {
    ...
    if (isOriginAKnot && !targetPin) {
      if (originPin?.isInput() && to > from + Configuration.distanceThreshold) {
        this.element.origin = /** @type {KnotPinTemplate} */
        originPin.template.getoppositePin();
    ...
    if (isTargetAKnot && !originPin) {
      if (targetPin?.isInputLoosely() && to < from - Configuration.distanceThreshold) {
        this.element.target = /** @type {KnotPinTemplate} */
        targetPin.template.getoppositePin();
    ...
    if (originPin && targetPin) {
      if (originPin.isKnot() && originPin.hasUpdated) {
        originPin.nodeElement.template.checkSwtichDirectionsVisually();
      }
      if (targetPin.isKnot() && targetPin.hasUpdated) {
        targetPin.nodeElement.template.checkSwtichDirectionsVisually();
      }
    }
    ...
    const xInverted = this.element.originatesFromInput ? this.element.originX < this.element.targetX : this.element.targetX < this.element.originX;
    this.element.startPixels = dx < width ? (width - dx) / 2 : 0;
    ...
    const c1 = startPercentage + (sameDirection ? 5 : (xInverted ? LinkTemplate.c1DecreasingValue(width) : 10) * fillRatio);
    const aspectRatio = dy / Math.max(30, dx);
    const c2 = sameDirection ? 100 * LinkTemplate.clampedLine(startPercentage / 50) + 15 : LinkTemplate.c2Clamped(dx) * LinkTemplate.sigmoidPositive(fillRatio * 1.2 + aspectRatio * 0.5, 1.5, 1.8) + startPercentage;
  ...
  update(changedProperties) {
    ...
    const mirrorV = (this.element.originY > this.element.targetY ? -1 : 1) * (this.element.originatesFromInput ? -1 : 1) * (this.element.origin?.isInputVisually() && this.element.target?.isInputVisually() ? -1 : 1);
...
class LinkElement extends IFromToPositionedElement {
  ...
  #nodeUpdateHandler = (e2) => {
    if (this.#origin.nodeElement === e2.target) {
    ...
    } else if (this.#target.nodeElement === e2.target) {
  ...
  #nodeDragOriginHandler = (e2) => this.addOriginLocation(...e2.detail.value);
  /** @param {UEBDragEvent} e */
  #nodeDragTargetHandler = (e2) => this.addTargetLocation(...e2.detail.value);
  #nodeReflowOriginHandler = (e2) => {
    if (this.origin.isKnot()) {
      this.originatesFromInput = this.origin.isInputVisually();
    }
    this.setOriginLocation();
  };
  #nodeReflowTargetHandler = (e2) => this.setTargetLocation();
  ...
  #setPin(pin, isTargetPin) {
    ...
    if (getCurrentPin()) {
      ...
      isTargetPin ? this.setTargetLocation() : (this.setOriginLocation(), this.originatesFromInput = this.origin.isInputVisually());
      this.#linkPins();
    }
  ...
  setOriginLocation(location = null, canPostpone = true) {
    if (location == null) {
      ...
      if (canPostpone && (!this.hasUpdated || !this.origin.hasUpdated)) {
        Promise.all([this.updateComplete, this.origin.updateComplete]).then(() => self.setOriginLocation(null, false));
        return;
      }
      location = this.origin.template.getLinkLocation();
    }
    const [x2, y3] = location;
    this.originX = x2;
    this.originY = y3;
  }
  /** @param {Coordinates} location */
  setTargetLocation(location = null, canPostpone = true) {
    if (location == null) {
      ...
      if (canPostpone && (!this.hasUpdated || !this.target.hasUpdated)) {
        Promise.all([this.updateComplete, this.target.updateComplete]).then(() => self.setTargetLocation(null, false));
        return;
      }
   ...
   * @param {PinElement} b
   */
  setMessageTypesIncompatible(a2, b$12) {
    this.linkMessageIcon = SVGIcon.reject;
    this.linkMessageText = b`${Utility.capitalFirstLetter(a2.pinType)} is not compatible with ${Utility.capitalFirstLetter(b$12.pinType)}.`;
  }
}
class IDraggableElement extends IElement {
   ...
   * @param {Number} y
   */
  setLocation(x2, y3, acknowledge = true) {
    const dx = x2 - this.locationX;
    const dy = y3 - this.locationY;
    this.locationX = x2;
    this.locationY = y3;
    if (this.blueprint && acknowledge) {
      const dragLocalEvent = new CustomEvent(
        /** @type {typeof IDraggableElement} */
        this.constructor.dragEventName,
        {
          detail: {
            value: [dx, dy]
          },
   ...
   * @param {Number} y
   */
  addLocation(x2, y3, acknowledge = true) {
    this.setLocation(this.locationX + x2, this.locationY + y3, acknowledge);
  }
  /** @param {Coordinates} value */
  acknowledgeDrag(value) {
    const dragEvent = new CustomEvent(
      /** @type {typeof IDraggableElement} */
      this.constructor.dragGeneralEventName,
      {
        detail: {
          value
        },
...
class IMouseClickDrag extends IPointing {
  ...
  #mouseMoveHandler = (e2) => {
    ...
    const location = this.setLocationFromEvent(e2);
    const movement = [e2.movementX, e2.movementY];
  ...
  listenEvents() {
    ...
    if (this.options.clickButton === Configuration.mouseRightClickButton) {
      this.#draggableElement.addEventListener("contextmenu", (e2) => e2.preventDefault());
    }
  }
  unlistenEvents() {
...
class MouseMoveDraggable extends IMouseClickDrag {
  ...
  clicked(location) {
    if (this.options.repositionOnClick) {
      this.target.setLocation(...this.stepSize > 1 ? Utility.snapToGrid(location[0], location[1], this.stepSize) : location);
...
function pinTitle(entity) {
  let result = entity.PinFriendlyName ? entity.PinFriendlyName.toString() : Utility.formatStringName(entity.PinName?.toString() ?? "");
  let match;
...
function nodeSubtitle(entity) {
  switch (entity.getType()) {
    case p$7.addDelegate:
    case p$7.clearDelegate:
    case p$7.callDelegate:
    case p$7.removeDelegate:
      return null;
  }
  const targetPin = entity.getPinEntities().find((pin) => !pin.isHidden() && pin.PinName?.toString() === "self" && pinTitle(pin) === "Target");
  if (targetPin) {
    const target = entity.FunctionReference?.MemberParent?.getName() ?? targetPin.PinType?.PinSubCategoryObject?.getName() ?? "Untitled";
...
class MouseMoveNodes extends MouseMoveDraggable {
  ...
  unclicked() {
    if (!this.started) {
      ...
      this.blueprint.getNodes(true).forEach(
        (node) => node.boundComments.filter((comment) => !node.isInsideComment(comment)).forEach((comment) => node.unbindFromComment(comment))
      );
      this.blueprint.getCommentNodes().forEach(
        (comment) => (
          /** @type {CommentNodeTemplate} */
          comment.template.manageNodesBind()
        )
      );
    }
  }
    ...
    return [
      ...
      new KeyboardShortcut(
        ...
        (self) => self.target.acknowledgeDrag([
          self.pressedKey === Configuration.Keys.ArrowLeft ? -Configuration.gridSize : self.pressedKey === Configuration.Keys.ArrowRight ? Configuration.gridSize : 0,
          self.pressedKey === Configuration.Keys.ArrowUp ? -Configuration.gridSize : self.pressedKey === Configuration.Keys.ArrowDown ? Configuration.gridSize : 0
        ])
      )
    ];
  }
...
class ISelectableDraggableTemplate extends IDraggablePositionedTemplate {
  ...
  createDraggableObject() {
    return (
      /** @type {MouseMoveDraggable} */
      new MouseMoveNodes(this.element, this.blueprint, {
...
class NodeTemplate extends ISelectableDraggableTemplate {
  ...
  createPinElement(pinEntity) {
    const pinElement = (
      /** @type {PinElementConstructor} */
      ElementFactory.getConstructor("ueb-pin").newObject(pinEntity, void 0, this.element)
    );
  ...
  initialize(element) {
    ...
    this.element.classList.add(.../** @type {typeof NodeTemplate} */
    this.constructor.nodeStyleClasses);
  ...
  setupPins() {
    this.element.nodeNameElement = /** @type {HTMLElement} */
    this.element.querySelector(".ueb-node-name-text");
    let hasInput = false;
    let hasOutput = false;
    for (const p2 of this.getPinElements()) {
      if (p2 === this.defaultPin) {
        continue;
      }
      if (p2.isInput()) {
        this.inputContainer.appendChild(p2);
        hasInput = true;
      } else if (p2.isOutput()) {
        this.outputContainer.appendChild(p2);
        hasOutput = true;
      }
    }
    if (this.defaultPin) {
  ...
  createPinElements() {
    return this.element.getPinEntities().filter((v2) => !v2.isHidden()).map((pinEntity) => this.createPinElement(pinEntity));
  }
  /** All the link connected to this node */
  getAllConnectedLinks() {
    const nodeTitle2 = this.element.nodeTitle;
    const query = `ueb-link[data-origin-node="${nodeTitle2}"],ueb-link[data-target-node="${nodeTitle2}"]`;
    return (
      /** @type {LinkElement[]} */
      [...this.blueprint.querySelectorAll(query)]
    );
  }
}
...
class CommentNodeTemplate extends IResizeableTemplate {
  ...
  manageNodesBind() {
    ...
    for (let node of nodes) {
      if (node.topBoundary() >= this.element.topBoundary() && node.rightBoundary() <= this.element.rightBoundary() && node.bottomBoundary() <= this.element.bottomBoundary() && node.leftBoundary() >= this.element.leftBoundary()) {
  ...
  bottomBoundary(justSelectableArea = false) {
    return justSelectableArea ? this.element.locationY + this.#selectableAreaHeight : super.bottomBoundary();
  }
  leftBoundary(justSelectableArea = false) {
    return this.element.locationX;
  }
...
class MouseCreateLink extends IMouseClickDrag {
  ...
  #mouseenterHandler = (e2) => {
    if (!this.enteredPin) {
      ...
      this.enteredPin = /** @type {PinElement} */
      e2.target;
      const a2 = this.link.origin ?? this.target;
      const b2 = this.enteredPin;
      const outputPin = a2.isOutput() ? a2 : b2;
      if (a2.isKnot() || b2.isKnot()) {
        this.link.setMessageCorrect();
        this.linkValid = true;
      } else if (a2.getNodeElement() === b2.getNodeElement()) {
        this.link.setMessageSameNode();
      } else if (a2.isOutput() === b2.isOutput()) {
        this.link.setMessageDirectionsIncompatible();
      } else if (this.blueprint.getLinks(a2, b2).length) {
        ...
        this.linkValid = true;
      } else if ((a2.entity.PinType.PinCategory.valueOf() != "object" || b2.entity.PinType.PinCategory.valueOf() != "object") && a2.pinType != b2.pinType) {
        this.link.setMessageTypesIncompatible(a2, b2);
        this.linkValid = false;
      } else {
        this.link.setMessageCorrect();
        this.linkValid = true;
  ...
  startDrag(location) {
    this.link = /** @type {LinkElementConstructor} */
    ElementFactory.getConstructor("ueb-link").newObject(this.target, null);
  ...
  endDrag() {
    ...
    if (this.enteredPin && this.linkValid) {
      const knot = this.enteredPin.isKnot() ? this.enteredPin : this.link.origin.isKnot() ? this.link.origin : null;
      if (knot) {
        ...
        if (this.link.origin.isOutput()) {
          this.enteredPin = /** @type {KnotPinTemplate} */
          this.enteredPin.template.getoppositePin();
        }
      }
...
class VariableManagementNodeTemplate extends NodeTemplate {
  ...
  createPinElements() {
    return this.element.getPinEntities().filter((v2) => !v2.isHidden()).map((v2) => {
      this.#hasInput ||= v2.isInput();
      this.#hasOutput ||= v2.isOutput();
      const result = (
        /** @type {PinElementConstructor} */
        ElementFactory.getConstructor("ueb-pin").newObject(v2, void 0, this.element)
      );
      return result;
    });
  }
...
class PinTemplate extends ITemplate {
  ...
  initialize(element) {
    ...
    if (this.element.nodeElement) {
      ...
      this.isNameRendered = !(nodeTemplate instanceof VariableConversionNodeTemplate || nodeTemplate instanceof VariableOperationNodeTemplate || nodeTemplate instanceof MetasoundOperationTemplate);
    }
  }
  setup() {
    super.setup();
    ...
    const nodeTemplate = this.element.nodeElement.template;
    if (nodeTemplate instanceof VariableConversionNodeTemplate || nodeTemplate instanceof VariableOperationNodeTemplate) {
  ...
  render() {
    ...
    return b`
            <div class="ueb-pin-wrapper">
                ${pcgSubobject && this.element.entity.getPinObject(pcgSubobject)?.["Properties"]?.["PinStatus"] == "Required" ? b`<div class="ueb-pin-required-mark"></div>` : A}
  ...
  renderIcon() {
    ...
    switch (this.element.entity.PinType.ContainerType?.toString()) {
      case "Array":
        return SVGIcon.arrayPin;
      case "Set":
        return SVGIcon.setPin;
      case "Map":
  ...
  renderName() {
    ...
    if (nodeElement.getType() == Configuration.paths.makeStruct && pinName == nodeElement.entity.StructType.getName()) {
      name = pinName;
    }
    return b`
  ...
  isInputRendered() {
    return this.element.isInput() && !this.element.entity.bDefaultValueIsIgnored?.valueOf() && !this.element.entity.PinType.bIsReference?.valueOf();
  }
  renderInput() {
    return b``;
  }
  ...
  getAllConnectedLinks() {
    ...
    const nodeTitle2 = this.element.nodeElement.nodeTitle;
    const pinId = this.element.pinId;
    const query = `ueb-link[data-origin-node="${nodeTitle2}"][data-origin-pin="${pinId}"],ueb-link[data-target-node="${nodeTitle2}"][data-target-pin="${pinId}"]`;
    return (
      /** @type {LinkElement[]} */
      [...this.blueprint.querySelectorAll(query)]
    );
...
class EventNodeTemplate extends NodeTemplate {
  ...
  renderTop() {
    const icon = this.renderNodeIcon();
    const name = this.renderNodeName();
    const customEvent = this.element.getType() === Configuration.paths.customEvent && (this.element.entity.CustomFunctionName || this.element.entity.FunctionReference.MemberParent);
    return b`
            <div class="ueb-node-name">
                ${icon ? b`
                    <div class="ueb-node-name-symbol">${icon}</div>
  ...
  getPinElements() {
    return this.element.getPinElements().filter((v2) => v2.entity.PinType.PinCategory?.toString() !== "delegate");
  }
  createDelegatePinElement() {
    if (!this.#delegatePinElement) {
      this.#delegatePinElement = /** @type {PinElementConstructor} */
      ElementFactory.getConstructor("ueb-pin").newObject(
        this.element.getPinEntities().find((v2) => !v2.isHidden() && v2.PinType.PinCategory?.toString() === "delegate"),
  ...
  createPinElements() {
    return [
      ...
      ...this.element.getPinEntities().filter((v2) => !v2.isHidden() && v2.PinType.PinCategory?.toString() !== "delegate").map(
        (pinEntity) => (
          /** @type {PinElementConstructor} */
          ElementFactory.getConstructor("ueb-pin").newObject(pinEntity, void 0, this.element)
        )
...
class KnotPinTemplate extends MinimalPinTemplate {
  ...
  update(changedProperties) {
    ...
    if (changedProperties.has("isLinked")) {
      ...
      if (!this.element.isLinked && !oppositePin.isLinked) {
        ...
        const type = this.element.getLinks().map((r2) => this.blueprint.getPin(r2)).find((p2) => p2 && p2.pinType != "wildcard")?.entity.PinType;
        if (type) {
          const propagated = [this];
          for (let i4 = 0; i4 < propagated.length; ++i4) {
            let current = propagated[i4];
            current.#setType(type);
            current = /** @type {KnotPinTemplate} */
            current.getoppositePin().template;
            current.#setType(type);
            propagated.push(
              ...current.element.getLinks().map((r2) => (
                /** @type {KnotPinTemplate} */
                this.blueprint.getPin(r2).template
              ))
            );
          }
        }
  ...
  getoppositePin() {
    const nodeTemplate = (
      /** @type {KnotNodeTemplate} */
      this.element.nodeElement.template
    );
...
class KnotNodeTemplate extends NodeTemplate {
  ...
  setupPins() {
    for (const p2 of this.getPinElements()) {
      this.element.querySelector(".ueb-node-border").appendChild(p2);
    }
  }
  createPinElements() {
    const entities = this.element.getPinEntities().filter((v2) => !v2.isHidden());
    const inputEntity = entities[entities[0].isInput() ? 0 : 1];
    const outputEntity = entities[entities[0].isOutput() ? 0 : 1];
    const pinElementConstructor = (
      /** @type {PinElementConstructor} */
      ElementFactory.getConstructor("ueb-pin")
    );
    let result = [
...
class VariableAccessNodeTemplate extends VariableManagementNodeTemplate {
  ...
  initialize(element) {
    ...
    if (type === Configuration.paths.variableGet || type === Configuration.paths.self) {
  ...
  setupPins() {
    super.setupPins();
    let outputPin = this.element.getPinElements().find((p2) => !p2.entity.isHidden() && !p2.entity.isExecution());
...
function nodeTemplateClass(nodeEntity) {
  ...
  if (className === p$6.callFunction || className === p$6.commutativeAssociativeBinaryOperator || className === p$6.callArrayFunction) {
    ...
    if (memberName && (memberParent === p$6.kismetArrayLibrary || memberParent === p$6.kismetMathLibrary || memberParent === p$6.kismetStringLibrary || memberParent === p$6.typedElementHandleLibrary)) {
      if (memberName.startsWith("Conv_")) {
        return VariableConversionNodeTemplate;
      }
      if (memberName.startsWith("Add_") || memberName.startsWith("And_") || memberName.startsWith("Boolean") || memberName.startsWith("Cross_") || memberName.startsWith("Dot_") || memberName.startsWith("Not_") || memberName.startsWith("Or_") || memberName.startsWith("Percent_") || memberName.startsWith("Xor_")) {
    ...
    if (memberParent === p$6.blueprintSetLibrary) {
      return VariableOperationNodeTemplate;
    }
    if (memberParent === p$6.blueprintMapLibrary) {
      return VariableOperationNodeTemplate;
    }
  }
  switch (className) {
    case p$6.comment:
    case p$6.materialGraphNodeComment:
...
const keyNameValue = {
  ...
  "E_AccentGrave": "è",
  "F1": "F1",
  // Otherwise F and number will be separated
  "F10": "F10",
  "F11": "F11",
  "F12": "F12",
  "F2": "F2",
...
const niagaraNodeNames = {
  ...
  ...Object.fromEntries(Object.entries({
    ...
    "Length": "Len",
    "Madd": `(A${String.fromCharCode(10799)}B)+C`,
    "Mul": String.fromCharCode(10799),
    "Negate": "-A",
    "OneMinus": "1-A",
    ...
    "TWO_PI": `2 ${String.fromCharCode(960)}`
  }).map(([k2, v2]) => ["Numeric::" + k2, v2]))
};
...
function nodeTitle(entity) {
  ...
  switch (entity.getType()) {
    ...
    case p$5.actorBoundEvent:
    case p$5.componentBoundEvent:
    ...
    case p$5.functionEntry:
      return entity.FunctionReference?.MemberName?.toString() === "UserConstructionScript" ? "Construction Script" : entity.FunctionReference?.MemberName?.toString();
    ...
    case p$5.materialExpressionComponentMask: {
      const materialObject2 = entity.getMaterialSubobject();
      if (materialObject2) {
        return `Mask ( ${Configuration.rgba.filter((k2) => (
          /** @type {MirroredEntity<typeof BooleanEntity>} */
          materialObject2[k2].getter().value === true
        )).map((v2) => v2 + " ").join("")})`;
      }
    }
    case p$5.materialExpressionConstant:
      value ??= [entity.getCustomproperties().find((pinEntity) => pinEntity.PinName.toString() == "Value")?.DefaultValue];
    case p$5.materialExpressionConstant2Vector:
      value ??= [
    ...
    case p$5.materialExpressionConstant3Vector:
    case p$5.materialExpressionConstant4Vector:
      if (!value) {
        const vector = entity.getCustomproperties().find((pinEntity) => pinEntity.PinName?.toString() == "Constant")?.DefaultValue;
        value = vector instanceof VectorEntity ? [vector.X, vector.Y, vector.Z].map((v2) => v2.valueOf()) : vector instanceof LinearColorEntity ? [vector.R, vector.G, vector.B, vector.A].map((v2) => v2.valueOf()) : (
          /** @type {Number[]} */
          []
        );
      }
      if (value?.length > 0) {
        return value.map((v2) => Utility.printExponential(v2)).join(",");
      }
      value = void 0;
      break;
    case p$5.materialExpressionFunctionInput: {
      const materialObject2 = entity.getMaterialSubobject();
      const inputName = materialObject2?.InputName ?? "In";
      const inputType = materialObject2?.InputType?.value.match(/^.+?_(\w+)$/)?.[1] ?? "Vector3";
      return `Input ${inputName} (${inputType})`;
    }
    case p$5.materialExpressionLogarithm:
      return "Ln";
    case p$5.materialExpressionLogarithm10:
      return "Log10";
    case p$5.materialExpressionLogarithm2:
      return "Log2";
    case p$5.materialExpressionMaterialFunctionCall:
      const materialFunction = entity.getMaterialSubobject()?.MaterialFunction;
      if (materialFunction) {
        return materialFunction.getName();
      }
      break;
    case p$5.materialExpressionSquareRoot:
      return "Sqrt";
    case p$5.materialExpressionSubtract:
      const materialObject = entity.getMaterialSubobject();
      if (materialObject) {
        return `Subtract(${materialObject.ConstA ?? "1"},${materialObject.ConstB ?? "1"})`;
      }
    case p$5.metasoundEditorGraphExternalNode: {
    ...
    case p$5.niagaraNodeConvert:
      const targetType = (entity["AutowireMakeType"]?.["ClassStructOrEnum"] ?? "").toString().match(/(?:Niagara)?(\w+)['"]*$/)?.[1] ?? "";
      return `Make ${targetType}`;
    case p$5.pcgEditorGraphNodeInput:
      return "Input";
    case p$5.pcgEditorGraphNodeOutput:
      return "Output";
    case p$5.soundNodeWavePlayer:
      return `Wave Player : ${entity.getSounCueSubobject()?.SoundWaveAssetPtr?.type.match(/([^.]+)$/)?.[0] ?? "NONE"}`;
    case p$5.spawnActorFromClass:
      let className2 = entity.getCustomproperties().find((pinEntity) => pinEntity.PinName.toString() == "ReturnValue")?.PinType?.PinSubCategoryObject?.getName();
      if (className2 === "Actor") {
        className2 = null;
      }
      return `SpawnActor ${format(className2 ?? "NONE")}`;
    case p$5.switchEnum:
      return `Switch on ${entity.Enum?.getName() ?? "Enum"}`;
    case p$5.switchInteger:
      return `Switch on Int`;
    case p$5.variableGet:
      return "";
    case p$5.variableSet:
      return "SET";
  }
  ...
  if (keyNameSymbol) {
    ...
    if (className === p$5.inputDebugKey) {
      title = "Debug Key " + title;
    } else if (className === p$5.getInputAxisKeyValue) {
      title = "Get " + title;
    }
    return title;
  }
  if (className === p$5.macro) {
  ...
  if (settingsObject) {
    if (settingsObject.ExportPath?.valueOf()?.type === p$5.pcgHiGenGridSizeSettings) {
      return `Grid Size: ${settingsObject.HiGenGridSize?.toString().match(/\d+/)?.[0]?.concat("00") ?? settingsObject.HiGenGridSize?.toString().match(/^\w+$/)?.[0] ?? "256"}`;
    }
  ...
  if (memberName) {
    ...
    if (memberNameTraceLineMatch) {
      return "Line Trace" + (memberNameTraceLineMatch[1] === "Multi" ? " Multi " : " ") + (memberNameTraceLineMatch[2] === "" ? "By Channel" : format(memberNameTraceLineMatch[2]));
    }
    switch (memberParent) {
      case p$5.blueprintGameplayTagLibrary:
      case p$5.kismetMathLibrary:
      case p$5.kismetStringLibrary:
      case p$5.slateBlueprintLibrary:
      case p$5.timeManagementBlueprintLibrary:
      case p$5.typedElementHandleLibrary:
  ...
  if (entity.OpName) {
    return niagaraNodeNames[entity.OpName.toString()] ?? format(entity.OpName.toString().replaceAll(new RegExp("(?:^\\w+(?<!^Matrix))?::", "g"), " "));
  }
  ...
  let prefix;
  if (className.startsWith(prefix = "/Script/NiagaraEditor.NiagaraNodeParameter") || className.startsWith(prefix = "/Script/NiagaraEditor.NiagaraNode")) {
...
class ISelectableDraggableElement extends IDraggableElement {
  ...
  /** @param {UEBDragEvent} e */
  dragHandler = (e2) => this.addLocation(...e2.detail.value);
  constructor() {
    super();
    this.selected = false;
...
class NodeElement extends ISelectableDraggableElement {
  ...
  set blueprint(v2) {
    super.blueprint = v2;
    this.#pins.forEach((p2) => p2.blueprint = v2);
  }
  /** @type {HTMLElement} */
  #nodeNameElement;
  get nodeNameElement() {
  ...
  isInsideComment(commentNode) {
    return this.topBoundary() >= commentNode.topBoundary() && this.rightBoundary() <= commentNode.rightBoundary() && this.bottomBoundary() <= commentNode.bottomBoundary() && this.leftBoundary() >= commentNode.leftBoundary();
  }
  getType() {
  ...
  setLocation(x2 = 0, y3 = 0, acknowledge = true) {
    this.entity.setNodePosX(x2);
    this.entity.setNodePosY(y3);
    super.setLocation(x2, y3, acknowledge);
  }
...
class BlueprintEntity extends ObjectEntity {
  ...
  renameScriptVariable(variable, entity) {
    ...
    {
      [true, false].forEach((v2) => {
        let object = this[Configuration.subObjectAttributeNameFromReference(variable, v2)];
  ...
  variableIndex(variableName) {
    let i4 = 0;
    for (const v2 of this.ScriptVariables?.valueOf()) {
      if (variableName == this.variableName(v2)) {
        return i4;
      }
      ++i4;
    }
  ...
  mergeWith(entity) {
    ...
    const variableObjectNames = this.ScriptVariables.valueOf().map((v2) => v2.ScriptVariable.getName());
    let scriptVariables = Utility.mergeArrays(
      this.ScriptVariables.valueOf(),
      entity.ScriptVariables.valueOf(),
      (l2, r2) => this.variableName(l2) == this.variableName(r2),
      (added) => {
    ...
    const entries = scriptVariables.concat(scriptVariables).map((v2, i4) => {
      const name = Configuration.subObjectAttributeNameFromReference(
        v2.ScriptVariable,
        i4 >= scriptVariables.length
    ...
    entries.push(
      ...Object.entries(this).filter(
        ([k2, v2]) => !k2.startsWith(Configuration.subObjectAttributeNamePrefix) && k2 !== "ExportedNodes"
      ),
  ...
  getVariablesAttributesReferringTo(...entities) {
    ...
    this.ScriptVariables.valueOf().map((v2) => {
      const keySimple = Configuration.subObjectAttributeNameFromReference(v2.ScriptVariable, false);
      const keyFull = Configuration.subObjectAttributeNameFromReference(v2.ScriptVariable, true);
      return {
        simple: [keySimple, this[keySimple]],
        full: [keyFull, this[keyFull]],
        variable: v2
      };
    }).filter((v2) => pins.has(v2.full?.["Variable"]?.["Name"])).reduce(
      (acc, cur) => {
...
class NiagaraClipboardContent extends ObjectEntity {
  ...
  constructor(blueprint, nodes) {
    ...
    const variableIndexes = /* @__PURE__ */ new Set();
    let exported = "";
    for (const node of nodes) {
      if (node.exported) {
        node.getPinEntities().map((pin) => blueprint.variableIndex(pin.PinName.toString())).filter((v2) => v2 != null).forEach((i4) => variableIndexes.add(i4));
        exported += node.serialize();
      }
    }
    const scriptVariables = blueprint.ScriptVariables.valueOf().filter((v2, i4) => variableIndexes.has(i4));
    const variableObjects = scriptVariables.concat(scriptVariables).map((v2, i4) => {
      const name2 = Configuration.subObjectAttributeNameFromReference(
        v2.ScriptVariable,
        i4 >= scriptVariables.length
        // First take all the small objects then all name only
      );
      return [name2, blueprint[name2]];
    });
    super({
...
class Paste extends IInput {
  ...
  constructor(target, blueprint, options = {}) {
    ...
    super(target, blueprint, options);
    let self = this;
    this.#pasteHandle = (e2) => self.pasted(e2.clipboardData.getData("Text"));
  }
  listenEvents() {
  ...
  pasted(value) {
    ...
    let nodes = ObjectEntity.grammarMultipleObjects.parse(value).map((entity) => {
      let node = (
        /** @type {NodeElementConstructor} */
        ElementFactory.getConstructor("ueb-node").newObject(entity)
      );
      top += node.locationY;
      left += node.locationX;
      ++count;
...
class MouseTracking extends IPointing {
  ...
  /** @param {CustomEvent} e */
  #trackingMouseStolenHandler = (e2) => {
    if (!this.#mouseTracker) {
      e2.preventDefault();
      this.#mouseTracker = e2.detail.tracker;
      this.unlistenMouseMove();
    }
  };
  /** @param {CustomEvent} e */
  #trackingMouseGaveBackHandler = (e2) => {
    if (this.#mouseTracker == e2.detail.tracker) {
      e2.preventDefault();
  ...
  listenEvents() {
    ...
    this.blueprint.addEventListener(
      ...
      /** @type {(e: Event) => any} */
      this.#trackingMouseStolenHandler
    );
    this.blueprint.addEventListener(
      Configuration.trackingMouseEventName.end,
      /** @type {(e: Event) => any} */
      this.#trackingMouseGaveBackHandler
    );
  }
  unlistenEvents() {
    this.unlistenMouseMove();
...
class Unfocus extends IInput {
  /** @param {MouseEvent} e */
  #clickHandler = (e2) => this.clickedSomewhere(
    /** @type {HTMLElement} */
    e2.target
  );
  /**
...
class BlueprintTemplate extends ITemplate {
  ...
  initialize(element) {
    super.initialize(element);
    this.element.style.cssText = Object.entries(BlueprintTemplate.styleVariables).map(([k2, v2]) => `${k2}:${v2};`).join("");
    const htmlTemplate = (
      /** @type {HTMLTemplateElement} */
      this.element.querySelector(":scope > template")?.content.textContent
    );
    if (htmlTemplate) {
      this.element.requestUpdate();
    ...
    return b`
            <div class="ueb-viewport-header">
                <div class="ueb-viewport-about">
                    <a @click="${(e2) => e2.target.closest("ueb-blueprint").querySelector(".ueb-info-dialog").showModal()}">
                        ⓘ
                    </a>
                </div>
            ...
            <dialog class="ueb-info-dialog" @click="${(e2) => e2.target.closest(".ueb-info-dialog").close()}">
                <h2>UEBlueprint</h2>
  ...
  willUpdate(changedProperties) {
    ...
    if (this.headerElement && changedProperties.has("zoom")) {
      if (this.headerElement.classList.contains("ueb-zoom-changed")) {
        ...
        void this.headerElement.offsetWidth;
      }
  ...
  updated(changedProperties) {
    ...
    if (changedProperties.has("zoom")) {
      ...
      const getClassName = (v2) => `ueb-zoom-${v2}`;
      if (previousZoom < this.blueprint.zoom) {
        this.blueprint.classList.remove(...classes.filter((v2) => v2 < 0).map(getClassName));
        this.blueprint.classList.add(...classes.filter((v2) => v2 > 0).map(getClassName));
      } else {
        this.blueprint.classList.remove(...classes.filter((v2) => v2 > 0).map(getClassName));
        this.blueprint.classList.add(...classes.filter((v2) => v2 < 0).map(getClassName));
      }
  ...
  getCommentNodes(justSelected = false) {
    return this.blueprint.querySelectorAll(
      `ueb-node[data-type="${Configuration.paths.comment}"]${justSelected ? '[data-selected="true"]' : ""}, ueb-node[data-type="${Configuration.paths.materialGraphNodeComment}"]${justSelected ? '[data-selected="true"]' : ""}`
    );
  }
  ...
  centerViewport(x2 = 0, y3 = 0, smooth = true) {
    ...
    this.blueprint.scrollDelta(
      this.blueprint.scaleCorrectReverse(x2 - centerX),
      this.blueprint.scaleCorrectReverse(y3 - centerY),
      smooth
    );
  }
  centerContentInViewport(smooth = true) {
    ...
    avgX = Math.round(
      maxX - minX <= this.viewportSize[0] ? (maxX + minX) / 2 : avgX / (2 * nodes.length)
    );
    avgY = Math.round(
      maxY - minY <= this.viewportSize[1] ? (maxY + minY) / 2 : avgY / (2 * nodes.length)
    );
...
class Blueprint extends IElement {
  ...
  scrollDelta(x2 = 0, y3 = 0, smooth = false, scrollTime = Configuration.smoothScrollTime) {
    if (smooth) {
      ...
      for (let i4 = 0; i4 < 2; ++i4) {
        if (finalScroll[i4] < Configuration.gridExpandThreshold * Configuration.expandGridSize) {
          expand[i4] = -1;
        } else if (finalScroll[i4] > maxScroll[i4] - Configuration.gridExpandThreshold * Configuration.expandGridSize) {
          expand[i4] = 1;
        }
      }
   ...
   */
  snapToGrid(x2, y3) {
    return Utility.snapToGrid(x2, y3, Configuration.gridSize);
  }
  /**
   * @param {Number} x
   * @param {Number} y
   */
  seamlessExpand(x2, y3) {
    x2 = Math.round(x2);
    y3 = Math.round(y3);
    let scale = this.getScale();
    {
      [x2, y3] = [-x2 * Configuration.expandGridSize, -y3 * Configuration.expandGridSize];
      if (x2 != 0) {
        this.scrollX += x2;
        x2 /= scale;
      }
  ...
  progressiveSnapToGrid(x2) {
    return Configuration.expandGridSize * Math.round(x2 / Configuration.expandGridSize + 0.5 * Math.sign(x2));
  }
  getZoom() {
    return this.zoom;
  }
  ...
  getNodes(selected = false) {
    ...
    if (selected) {
      result = result.filter((n3) => n3.selected);
    }
    return result;
  }
  getCommentNodes(justSelected = false) {
    let result = (
      /** @type {NodeElement[]} */
      [...this.template.getCommentNodes(justSelected)]
    );
    if (result.length === 0) {
      result = this.nodes.filter(
        (n3) => n3.getType() === Configuration.paths.comment && (!justSelected || n3.selected)
      );
    }
    return result;
  }
  ...
  getPin(pinReference) {
    ...
    if (!result || result.nodeElement.getNodeName() != pinReference.objectName.toString()) {
      result = [...this.nodes.find((n3) => pinReference.objectName.toString() == n3.getNodeName())?.getPinElements() ?? []].find((p2) => pinReference.pinGuid.toString() == p2.getPinId().toString());
    }
    return result;
  }
  /**
  ...
  getLinks(a2 = null, b2 = null) {
    ...
    if (a2 != null && b2 != null) {
      return this.links.filter(
        (link) => link.origin == a2 && link.target == b2 || link.origin == b2 && link.target == a2
      );
    }
    return this.links;
  }
  ...
  getLink(originPin, targetPin, strictDirection = false) {
    return this.links.find(
      (link) => link.origin == originPin && link.target == targetPin || !strictDirection && link.origin == targetPin && link.target == originPin
    );
  }
  selectAll() {
  ...
  getSerializedText() {
    const nodes = this.blueprint.getNodes(true).map((n3) => n3.entity);
    let exports$1 = false;
    let result = nodes.filter((n3) => {
      exports$1 ||= n3.exported;
      return !n3.exported;
  ...
  addGraphElement(...graphElements) {
    const removeEventHandler = (event) => {
      ...
      const [container, entity] = target instanceof NodeElement ? [this.nodes, target.entity] : target instanceof LinkElement ? [this.links] : null;
      ...
      if (element instanceof NodeElement && !this.nodes.includes(element)) {
        ...
        if (element.getType() == Configuration.paths.niagaraClipboardContent) {
          ...
          if (additionalSerialization) {
            this.template.getPasteInputObject().pasted(additionalSerialization).forEach((node) => node.entity.exported = true);
          }
          continue;
        }
    ...
    graphElements.filter((element) => element instanceof NodeElement).forEach(
      (node) => (
        /** @type {NodeElement} */
        node.sanitizeLinks(graphElements)
      )
    );
    graphElements.filter((element) => element instanceof NodeElement && element.getType() == Configuration.paths.comment).forEach((element) => element.updateComplete.then(
      () => (
        /** @type {CommentNodeTemplate} */
        element.template.manageNodesBind()
      )
    ));
  }
  /** @param  {...IElement} graphElements */
  ...
  acknowledgeEditText(begin) {
    const event = new CustomEvent(
      begin ? Configuration.editTextEventName.begin : Configuration.editTextEventName.end
    );
    this.dispatchEvent(event);
  }
}
...
class IDraggableControlTemplate extends IDraggableTemplate {
   ...
   * @returns {Coordinates}
   */
  adjustLocation(x2, y3) {
    this.locationChangeCallback?.(x2, y3);
    return [x2, y3];
  }
}
class ColorHandlerTemplate extends IDraggableControlTemplate {
  ...
  adjustLocation(x2, y3) {
    ...
    x2 = x2 - radius;
    y3 = -(y3 - radius);
    let [r2, theta] = Utility.getPolarCoordinates(x2, y3);
    r2 = Math.min(r2, radius), [x2, y3] = Utility.getCartesianCoordinates(r2, theta);
    this.locationChangeCallback?.(x2 / radius, y3 / radius);
    x2 = Math.round(x2 + radius);
    y3 = Math.round(-y3 + radius);
    return [x2, y3];
  }
}
class IDraggableControlElement extends IDraggableElement {
   ...
   */
  setLocation(x2, y3) {
    super.setLocation(...this.template.adjustLocation(x2, y3));
  }
}
...
class ColorSliderTemplate extends IDraggableControlTemplate {
   ...
   */
  adjustLocation(x2, y3) {
    x2 = Utility.clamp(x2, 0, this.movementSpaceSize[0]);
    y3 = Utility.clamp(y3, 0, this.movementSpaceSize[1]);
    this.locationChangeCallback?.(x2 / this.movementSpaceSize[0], 1 - y3 / this.movementSpaceSize[1]);
    return [x2, y3];
  }
}
...
class DropdownTemplate extends ITemplate {
  ...
  #changeHandler = (e2) => this.element.selectedOption = /** @type {HTMLSelectElement} */
  e2.target.selectedOptions[0].value;
  render() {
    return b`
            <select class="ueb-pin-input-content" @change="${this.#changeHandler}">
                ${this.element.options.map(([k2, v2]) => b`
                    <option value="${k2}" ?selected="${k2 === this.element.selectedOption}">${v2}</option>
                `)}
            </select>
  ...
  setSelectedValue(value) {
    this.element.querySelector(`option[value="${value}"]`).defaultSelected = true;
  }
  getSelectedValue() {
...
class InputTemplate extends ITemplate {
  ...
  /** @param {Event} e */
  #inputSingleLineHandler = (e2) => (
    /** @type {HTMLElement} */
    e2.target.querySelectorAll("br").forEach((br) => br.remove())
  );
  /** @param {KeyboardEvent} e */
  #onKeydownBlurOnEnterHandler = (e2) => {
    if (e2.code == "Enter" && !e2.shiftKey) {
      e2.target.blur();
    }
  };
...
class IInputPinTemplate extends PinTemplate {
  ...
  static saveEachInputChange = false;
  // Otherwise save only on focus out
  /** @type {HTMLElement} */
  #inputWrapper;
  get inputWrapper() {
  ...
  static stringFromUEToInput(value) {
    return value.replaceAll(new RegExp("(?:\\r|(?<=(?:^|[^\\\\])(?:\\\\\\\\)*)\\\\r)(?=\\n)", "g"), "").replace(new RegExp("(?<=\\n\\s*)$"), "\n");
  }
  #setInput = () => this.setInputs(this.getInputs(), true);
  /** @param {Event} event */
  #checkWrapHandler = (event) => this.#updateWrapClass(
    /** @type {HTMLElement} */
    event.target
  );
  ...
  firstUpdated(changedProperties) {
    ...
    [...this.element.querySelectorAll("ueb-input")];
  }
  setup() {
    super.setup();
    const Self = (
  ...
  getInputs() {
    return this.#inputContentElements.map(
      (element) => (
  ...
  setInputs(values = [], updateDefaultValue = true) {
    this.#inputContentElements.forEach(
      /** @type {typeof IInputPinTemplate } */
      this.constructor.singleLineInput ? (elem, i4) => elem.innerText = values[i4] : (elem, i4) => elem.innerText = values[i4].replaceAll("\n", "")
    );
    if (updateDefaultValue) {
      this.setDefaultValue(values.map((v2) => IInputPinTemplate.stringFromInputToUE(v2)), values);
    }
    this.element.requestUpdate();
...
class EnumPinTemplate extends IInputPinTemplate {
  static saveEachInputChange = true;
  // Otherwise save only on focus out
  ...
  setup() {
    ...
    this.#dropdownEntries = enumEntries?.map((k2) => {
      if (k2.valueOf() === "") {
        k2 = new StringEntity("None");
      }
      return [
        k2,
        this.element.nodeElement.getPinEntities().find((pinEntity) => k2 === pinEntity.PinName)?.PinFriendlyName.toString() ?? k2
      ];
    }) ?? Configuration.CommonEnums[this.element.entity.getSubCategory()]?.map(
      (k2) => k2 instanceof Array ? k2 : [k2, Utility.formatStringName(k2)]
    ) ?? [];
    const defaultEntry = this.element.getDefaultValue()?.toString();
    if (!this.#dropdownEntries.find(([k2, v2]) => k2 === defaultEntry)) {
...
class WindowTemplate extends IDraggablePositionedTemplate {
  ...
  getDraggableElement() {
    return (
      /** @type {WindowElement} */
      this.element.querySelector(".ueb-window-top")
    );
  }
  createDraggableObject() {
    return new MouseMoveDraggable(this.element, this.blueprint, {
...
class ColorPickerWindowTemplate extends WindowTemplate {
  ...
  firstUpdated(changedProperties) {
    ...
    this.#saturationSlider.template.locationChangeCallback = /** @param {Number} x in the range [0, 1] */
    (x2, y3) => {
      this.color.setFromHSVA(this.color.H.value, y3, this.color.V.value, this.color.A.value);
      this.element.requestUpdate();
    };
    this.#valueSlider.template.locationChangeCallback = /** @param {Number} x in the range [0, 1] */
    (x2, y3) => {
      this.color.setFromHSVA(this.color.H.value, this.color.S.value, y3, this.color.A.value);
      this.element.requestUpdate();
    };
    this.#rSlider.template.locationChangeCallback = /** @param {Number} x in the range [0, 1] */
    (x2, y3) => {
      this.color.setFromRGBA(x2, this.color.G.value, this.color.B.value, this.color.A.value);
      this.element.requestUpdate();
    };
    this.#gSlider.template.locationChangeCallback = /** @param {Number} x in the range [0, 1] */
    (x2, y3) => {
      this.color.setFromRGBA(this.color.R.value, x2, this.color.B.value, this.color.A.value);
      this.element.requestUpdate();
    };
    this.#bSlider.template.locationChangeCallback = /** @param {Number} x in the range [0, 1] */
    (x2, y3) => {
      this.color.setFromRGBA(this.color.R.value, this.color.G.value, x2, this.color.A.value);
      this.element.requestUpdate();
    };
    this.#aSlider.template.locationChangeCallback = /** @param {Number} x in the range [0, 1] */
    (x2, y3) => {
      this.color.setFromRGBA(this.color.R.value, this.color.G.value, this.color.B.value, x2);
      this.element.requestUpdate();
    };
    this.#hSlider.template.locationChangeCallback = /** @param {Number} x in the range [0, 1] */
    (x2, y3) => {
      this.color.setFromHSVA(x2, this.color.S.value, this.color.V.value, this.color.A.value);
      this.element.requestUpdate();
    };
    this.#sSlider.template.locationChangeCallback = /** @param {Number} x in the range [0, 1] */
    (x2, y3) => {
      this.color.setFromHSVA(this.color.H.value, x2, this.color.V.value, this.color.A.value);
      this.element.requestUpdate();
    };
    this.#vSlider.template.locationChangeCallback = /** @param {Number} x in the range [0, 1] */
    (x2, y3) => {
      this.color.setFromHSVA(this.color.H.value, this.color.S.value, x2, this.color.A.value);
      this.element.requestUpdate();
    };
  }
  ...
  renderSlider(channel) {
    ...
    let channelValue = 0;
    let background = "";
    const getCommonBackground = (channel2) => `linear-gradient(to right, #${this.#colorHexReplace(channel2, "00", true)}, #${this.#colorHexReplace(channel2, "ff", true)})`;
    switch (channel) {
      case 0:
        channelLetter = "r";
      ...
      case 5:
        ...
        background = `linear-gradient(to right,#${this.#tempColor.setFromHSVA(
          this.color.H.value,
          0,
          this.color.V.value,
          1
        ), this.#tempColor.toRGBAString()},#${this.#tempColor.setFromHSVA(
          this.color.H.value,
          1,
          this.color.V.value,
          1
...
class LinearColorPinTemplate extends PinTemplate {
  ...
  #window;
  /** @param {MouseEvent} e */
  #launchColorPickerWindow = (e2) => {
    e2.preventDefault();
    this.blueprint.setFocused(true);
    this.#window = /** @type {WindowElementConstructor} */
    ElementFactory.getConstructor("ueb-window").newObject({
...
class RotatorPinTemplate extends INumericPinTemplate {
  ...
  setDefaultValue(values = [], rawValues = values) {
    ...
    rotator.R.value = values[0];
    rotator.P.value = values[1];
    rotator.Y.value = values[2];
...
const inputPinTemplates = {
  ...
  [p$4.linearColor]: LinearColorPinTemplate,
  [p$4.niagaraBool]: BoolPinTemplate,
  [p$4.niagaraFloat]: RealPinTemplate,
  [p$4.niagaraInt32]: IntPinTemplate,
  [p$4.niagaraPosition]: VectorPinTemplate,
  [p$4.rotator]: RotatorPinTemplate,
  [p$4.vector]: VectorPinTemplate,
  [p$4.vector2D]: Vector2DPinTemplate,
  [p$4.vector2f]: Vector2DPinTemplate,
  [p$4.vector3f]: VectorPinTemplate,
  [p$4.vector4f]: Vector4DPinTemplate
};
...
class PinElement extends IElement {
  static properties = {
    pinId: {
      type: GuidEntity,
      converter: {
        fromAttribute: (value, type) => value ? GuidEntity.grammar.parse(value) : null,
        toAttribute: (value, type) => value?.toString()
      },
      attribute: "data-id",
      reflect: true
    ...
    color: {
      type: LinearColorEntity,
      converter: {
        fromAttribute: (value, type) => value ? LinearColorEntity.getLinearColorFromAnyFormat().parse(value) : null,
        /** @param {LinearColorEntity} value */
        toAttribute: (value, type) => value?.toString() ?? ""
      },
  ...
  /** @type {NodeElement} */
  nodeElement;
  static newObject(entity = new PinEntity(), template = (
    /** @type {PinTemplate} */
    new (pinTemplate(entity))()
  ), nodeElement = void 0) {
  ...
  isInputLoosely() {
    return this.isInput(false) && this.isInput(true) === void 0;
  }
  /** @returns {boolean} True when the pin is input and if it is a knot it appears input */
  isInputVisually() {
    const template = (
      /** @type {KnotNodeTemplate} */
      this.nodeElement.template
    );
    const isKnot = this.isKnot();
    return isKnot && this.isInput() != template.switchDirectionsVisually || !isKnot && this.isInput();
  }
  ...
  isOutputLoosely() {
    return this.isOutput(false) && this.isOutput(true) === void 0;
  }
  ...
  isOutputVisually() {
    ...
    const isKnot = this.isKnot();
    return isKnot && this.isOutput() != template.switchDirectionsVisually || !isKnot && this.isOutput();
  }
    ...
    this.entity.LinkedTo = new PinEntity.attributes.LinkedTo(
      this.entity.LinkedTo?.valueOf().filter((pinReference) => {
        ...
        if (pin) {
          ...
          if (!link) {
            link = /** @type {LinkElementConstructor} */
            ElementFactory.getConstructor("ueb-link").newObject(this, pin);
            this.blueprint.addGraphElement(link);
          }
        }
        return pin;
  ...
  linkTo(targetPinElement) {
    ...
    if (this.isLinked && this.entity.isExecution() && this.isOutput(true) && this.getLinks().some((ref) => !pinReference.equals(ref))) {
      if (this.isKnot()) ;
      this.unlinkFromAll();
    }
  ...
  unlinkFrom(targetPinElement, removeLink = true) {
    if (this.entity.unlinkFrom(targetPinElement.getNodeElement().getNodeName(), targetPinElement.entity)) {
      this.isLinked = this.entity.isLinked();
      if (removeLink) {
        this.blueprint.getLink(this, targetPinElement)?.remove();
      }
      if (this.entity.recomputesNodeTitleOnChange) {
        this.nodeElement?.computeNodeDisplayName();
      }
  ...
  redirectLink(originalPinElement, newReference) {
    const index = this.getLinks().findIndex(
      (pinReference) => pinReference.objectName.toString() == originalPinElement.getNodeElement().getNodeName() && pinReference.pinGuid.toString() == originalPinElement.entity.PinId.toString()
    );
    if (index >= 0) {
...
class OrderedIndexArray {
   ...
   * @param {number} value
   */
  constructor(comparisonValueSupplier = (v2) => v2, value = null) {
  ...
  getPosition(value) {
    ...
    while (l2 < r2) {
      ...
      if (this.comparisonValueSupplier(this.array[m2]) < value) {
        l2 = m2 + 1;
      } else {
        r2 = m2;
      }
  ...
  insert(element, comparisonValue = null) {
    ...
    if (position < this.currentPosition || comparisonValue != null && position == this.currentPosition && this.comparisonValueSupplier(element) < comparisonValue) {
      ...
      if (this.metadata[index].onSecondaryAxis) {
        ...
        if (added) {
          ...
          if (
            ...
            Math.sign(finalPosition[1] - secondaryBoundary) == direction[1] && Math.sign(secondaryBoundary - this.initialPosition[1]) == direction[1]
          ) {
...
class SelectorElement extends IFromToPositionedElement {
  ...
  beginSelect(initialPosition) {
    const blueprintConstructor = (
      /** @type {BlueprintConstructor} */
      this.blueprint.constructor
    );
  ...
  static properties = {
    ...
    type: {
      ...
      converter: {
        ...
        toAttribute: (value, type) => Object.entries(WindowElement.#typeTemplateMap).find(([k2, v2]) => value.constructor === v2)?.[0]
      }
    }
  };
...
const _UnknownKeysEntity = class _UnknownKeysEntity extends IEntity {
  ...
  static createGrammar() {
    return Parsernostrum.seq(
      ...
      Parsernostrum.seq(Grammar.attributeName, Grammar.equalSeparation).map(([attribute, equal]) => attribute).chain(
...
function initializeSerializerFactory() {
  IEntity.unknownEntityGrammar = Parsernostrum.alt(
    ...
    Parsernostrum.alt(
      ...
      )).map(([_0, type, path]) => new ObjectReferenceEntity(type, path, (t2, p2) => `"${t2}'${p2}'"`))
    ),
...
function nodeColor(entity) {
  switch (entity.getType()) {
    ...
    case p$3.materialExpressionTextureCoordinate:
    case p$3.materialExpressionWorldPosition:
    ...
    case p$3.materialExpressionTextureSample:
      return Configuration.nodeColors.darkTurquoise;
    case p$3.niagaraNodeInput:
      switch (entity["Usage"]?.toString()) {
        case "Attribute":
          return Configuration.nodeColors.intenseGreen;
        case "Parameter":
          return Configuration.nodeColors.red;
        case "RapidIterationParameter":
          return Configuration.nodeColors.black;
        case "SystemConstant":
          return Configuration.nodeColors.gray;
        case "TranslatorConstant":
          return Configuration.nodeColors.gray;
        default:
          return Configuration.nodeColors.red;
      }
  ...
  if (entity.isComment()) {
    return (entity.CommentColor ? entity.CommentColor : LinearColorEntity.getWhite()).toDimmedColor().toCSSRGBValues();
  }
...
function nodeIcon(entity) {
  ...
  switch (entity.getType()) {
    ...
    case p$2.removeDelegate:
      return SVGIcon.node;
    case p$2.customEvent:
      return SVGIcon.event;
    case p$2.doN:
      return SVGIcon.doN;
    case p$2.doOnce:
      return SVGIcon.doOnce;
    case p$2.dynamicCast:
      return SVGIcon.cast;
    case p$2.enumLiteral:
      return SVGIcon.enum;
    case p$2.event:
      return SVGIcon.event;
    ...
    case p$2.ifThenElse:
      return SVGIcon.branchNode;
    case p$2.isValid:
      return SVGIcon.questionMark;
    case p$2.makeArray:
      return SVGIcon.makeArray;
    case p$2.makeMap:
      return SVGIcon.makeMap;
    case p$2.makeSet:
      return SVGIcon.makeSet;
    case p$2.makeStruct:
      return SVGIcon.makeStruct;
    case p$2.metasoundEditorGraphExternalNode:
      return SVGIcon.metasoundFunction;
    case p$2.select:
      return SVGIcon.select;
    case p$2.spawnActorFromClass:
      return SVGIcon.spawnActor;
    case p$2.timeline:
      return SVGIcon.timer;
  }
  if (entity.switchTarget()) {
...
const indexFromUpperCaseLetterName = (pinEntity) => pinEntity.PinName?.toString().match(/^\s*([A-Z])\s*$/)?.[1]?.charCodeAt(0) - "A".charCodeAt(0);
...
function nodeVariadic(entity) {
  ...
  let name;
  switch (type) {
    case p$1.commutativeAssociativeBinaryOperator:
    case p$1.promotableOperator:
      name = entity.FunctionReference?.MemberName?.toString();
      switch (name) {
        default:
          if (!name?.startsWith("Add_") && !name?.startsWith("Subtract_") && !name?.startsWith("Multiply_") && !name?.startsWith("Divide_")) {
            break;
          }
        case "And_Int64Int64":
        case "And_IntInt":
    ...
    case p$1.multiGate:
      ...
      pinNameFromIndex ??= (index, min = -1, max = -1, newPin) => `${prefix} ${index >= 0 ? index : min > 0 ? `${prefix} 0` : max + 1}`;
      break;
  ...
  if (pinEntities) {
    return () => {
      ...
      values.sort((a2, b2) => a2 < b2 ? -1 : a2 === b2 ? 0 : 1);
      let prev = values[0];
...
const colors = {
  ...
  [p.linearColor]: i$5`0, 88, 200`,
  [p.niagaraBool]: i$5`146, 0, 0`,
  [p.niagaraDataInterfaceCollisionQuery]: i$5`0, 168, 242`,
  [p.niagaraDataInterfaceCurlNoise]: i$5`0, 168, 242`,
  [p.niagaraDataInterfaceVolumeTexture]: i$5`0, 168, 242`,
  [p.niagaraFloat]: i$5`160, 250, 68`,
  [p.niagaraInt32]: i$5`30, 224, 172`,
  [p.niagaraPosition]: i$5`251, 146, 251`,
  [p.quat4f]: i$5`0, 88, 200`,
  [p.rotator]: i$5`157, 177, 251`,
  [p.transform]: i$5`227, 103, 0`,
  [p.vector]: i$5`251, 198, 34`,
  [p.vector2f]: i$5`0, 88, 200`,
  [p.vector3f]: i$5`250, 200, 36`,
  [p.vector4f]: i$5`0, 88, 200`
};
...
function pinColor(entity) {
  ...
  const type = entity.getType();
  return colors[type] ?? colors[entity.PinType.PinCategory?.toString().toLowerCase()] ?? (type.startsWith("/Script/Niagara.") ? colors["struct"] : colors["default"]);
}
